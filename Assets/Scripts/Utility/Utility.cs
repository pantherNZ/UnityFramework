...
public static partial class Utility
{
    ...
    public static void GetResourcePaths( string path, ref List<string> pathsOut, bool recursive = true, string extension = ".asset" )
    {
        if ( path.EndsWith( '/' ) )
            path = path[0..^1];
        ...
        foreach ( var file in dirInfo.GetFiles() )
            if ( file.Name.EndsWith( extension ) )
                pathsOut.Add( path + "/" + file.Name[0..^extension.Length] );
    ...
    static GameObject DrawCircleInternal( Vector3 position, float diameter, float lineWidth, Color? colour, float duration, int segments, EAxis axis )
    {
        var newObj = new GameObject( "DebugCircle" );
    ...
    {
        var sphere = new GameObject( "DebugSphere" );
        DrawCircleInternal( position, diameter, lineWidth, colour, duration, segments, EAxis.X ).transform.SetParent( sphere.transform );
        DrawCircleInternal( position, diameter, lineWidth, colour, duration, segments, EAxis.Y ).transform.SetParent( sphere.transform );
        DrawCircleInternal( position, diameter, lineWidth, colour, duration, segments, EAxis.Z ).transform.SetParent( sphere.transform );
    }
    ...
    public static void DrawRect( RectInt rect, Color? colour = null, float duration = 0.01f ) => DrawRect( rect.ToRect(), colour, duration );
    ...
    public static void DrawRectXZ( RectInt rect, Color? colour = null, float duration = 0.01f, float height = 0.0f ) => DrawRectXZ( rect.ToRect(), colour, duration, height );
