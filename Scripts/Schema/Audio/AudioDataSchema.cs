...
namespace Schema.Audio
{
	...
	[CreateAssetMenu( fileName = "AudioDataSchema", menuName = "NQ/Audio/AudioDataSchema" )]
	public class AudioDataSchema : BaseDataSchema
	{
		...
		[Tooltip( "If set, we don't generate a specific enum entry for this and instead we play it via the group" )]
		public AudioDataSchema groupedWith;
		public float pitchMultiplier = 1.0f;
		[Tooltip( "Minimum seconds that must elapse before this sound can play again. 0 = no cooldown." )]
		public float cooldownTimeSec = 0.0f;
		[Tooltip( "When this sound plays, all other sounds that share any of these cooldown groups will also be put on cooldown for their own cooldownTimeSec." )]
		public List<AudioDataSchema> cooldownGroups = new();
